...
public class BoardManager implements Listener {
    ...
    private BannerBoardPlugin plugin;
    ...
    public Listener init(BannerBoardPlugin plugin) {
        this.plugin = plugin;
        return this;
    }
    ...
    public void createBoard(Player player) {
        ...
        BannerBoardPlugin.msg(player, "&DThe &YBannerBoard creation toolkit &Dhas been added to your inventory.");
    }
    ...
    private void onBreak(BlockBreakEvent event) {
        ...
        while (it.hasNext()) {
            if (it.next().equals(event.getBlock().getLocation())) {
                it.remove();
                BannerBoardPlugin.msg(event.getPlayer(), "&DFirst location removed.");
            }
        }
    }
        ...
        if (event.getItemInHand() != null && event.getItemInHand().hasItemMeta() && event.getItemInHand().getItemMeta().hasDisplayName()) {
            ...
            if (name.equals(BANNERBOARD_CREATOR)) {
                ...
                if (!(event.getPlayer().isOp() || event.getPlayer().hasPermission("bannerboard.create"))) {
                    BannerBoardPlugin.msg(event.getPlayer(), "&DYou are not allowed to do that. Please contact a server administrator if you think this might be a mistake.");
                    return;
                }
                ...
                if (!this.previousBlock.containsKey(event.getPlayer())) {
                    ...
                    BannerBoardPlugin.msg(event.getPlayer(), "&DFirst location set at &YX=" + loc.getBlockX() + " Y=" + loc.getBlockY() + " Z=" + loc.getBlockZ() + "&D.");
                } else {
                    Location loc1 = this.previousBlock.remove(event.getPlayer());
                    Location loc2 = event.getBlock().getLocation();

                    BannerBoardPlugin.msg(event.getPlayer(), "&DSetting up a new bannerboard, please wait...");

                    if (!loc1.getWorld().equals(loc2.getWorld())) {
                        BannerBoardPlugin.msg(event.getPlayer(), "&DCannot generate a &YBannerBoard &Dwithin two different worlds.");
                        return;
                    }

                    if (loc1.distanceSquared(loc2) > 16 * 16) {
                        BannerBoardPlugin.msg(event.getPlayer(), ChatColor.RED + "The BannerBoard you are currently creating is very big! Your players will most likely have a dropped framerate when they are close to this banner. Make sure you know what you are doing, this might result in something bad!");
                    }

                    if (loc1.getBlockX() != loc2.getBlockX() && //
                            loc1.getBlockZ() != loc2.getBlockZ() && //
                            loc1.getBlockY() != loc2.getBlockY()) {
                        BannerBoardPlugin.msg(event.getPlayer(), "&DFailed to create BannerBoard, error &Y[The thinkness of the selection has to be 1 in at least one dimension]");
                        event.setCancelled(true);
                        return;
                    }
                    ...
                    if (hasWall == null) {
                        BannerBoardPlugin.msg(event.getPlayer(), "&DFailed to create BannerBoard, error &Y[There is no solid wall behind the BannerBoard]");
                        event.setCancelled(true);
                        return;
                    }
                        ...
                        if (e instanceof Hanging) {
                            for (Location frame : bannerBoard) {
                                if (equals(frame, e.getLocation())) {
                                    BannerBoardPlugin.msg(event.getPlayer(), "&DFailed to create BannerBoard, error &Y[Surface may not contain any hanging entities]");
                                    event.setCancelled(true);
                                    return;
                                }
                            }
                    ...
                    for (Location loc : bannerBoard) {
                        if (loc.getBlock().getType() != Material.AIR && !loc.equals(loc1) && !loc.equals(loc2)) {
                            BannerBoardPlugin.msg(event.getPlayer(), "&DFailed to create BannerBoard, error &Y[There were blocks in the way]");
                            event.setCancelled(true);
                            return;
                        }
                    }
                    ...
                    BannerBoardPlugin.msg(event.getPlayer(), "&DSuccessfully generated board &Y" + newId + " &Dbetween points &Y[" + loc1.getBlockX() + "," + loc1.getBlockY() + "," + loc1.getBlockZ() + "] and [" + loc2.getBlockX() + "," + loc2.getBlockY() + "," + loc2.getBlockZ()
