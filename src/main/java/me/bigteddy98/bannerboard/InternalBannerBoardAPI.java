...
public class InternalBannerBoardAPI implements BannerBoardAPI {
    ...
    public void registerPlaceHolder(String name, PlaceHolder placeHolder) {
        check();
        nameCheck(name);
        BannerBoardPlugin.getInstance().placeHolderManager.registerPlaceHolder(name, placeHolder);
    }

    @Override
    public Map<String, PlaceHolder> getRegisteredPlaceHolders() {
        return BannerBoardPlugin.getInstance().placeHolderManager.getReadOnlyCopy();
    }
    ...
    public void registerCustomRenderer(String name, CustomRenderer customRenderer) {
        check();
        nameCheck(name);
        BannerBoardPlugin.getInstance().rendererManager.registerRenderer(name, customRenderer);
    }
    ...
    public Map<String, CustomRenderer> getRegisteredRenderers() {
        return BannerBoardPlugin.getInstance().rendererManager.getReadOnlyCopy();
    }
    ...
    public BufferedImage getLoadedImage(String fileName) {
        if (!BannerBoardPlugin.getInstance().cachedImages.containsKey(fileName)) {
        ...
        return BannerBoardPlugin.getInstance().cachedImages.get(fileName);
    }

    @Override
    public boolean hasLoadedImage(String fileName) {
        return BannerBoardPlugin.getInstance().cachedImages.containsKey(fileName);
    }
    ...
    public String applyPlaceholders(Player p, String text) {
        return BannerBoardPlugin.getInstance().applyPlaceholders(text, p);
    }

    @Override
    public int getFrameIndex(ItemFrame frame) {
        check();
        for (BannerBoard board : BannerBoardPlugin.getInstance().memoryManager.getLoadedBannerBoards()) {
    ...
    public BufferedImage fetchImage(String url) {
        try {
            return BannerBoardPlugin.getInstance().fetchImage(url);
        } catch (IOException e) {
