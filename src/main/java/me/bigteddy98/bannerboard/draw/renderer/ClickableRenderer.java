    ...
    public void handle(Action action, final Player clicker, int boardWidth, int boardHeight, int frameIndex, int bannerId) {
        ...
        if (xOffset >= frameX0 && xOffset < frameX1 && yOffset >= frameY0 && yOffset < frameY1) {
            ...
            if (this.hasSetting("text")) {
                ...
                text = BannerBoardPlugin.getInstance().applyPlaceholders(text, clicker);
            ...
            if (this.hasSetting("consolecommand")) {
                ...
                // apply placeholders
                command = BannerBoardPlugin.getInstance().applyPlaceholders(command, clicker);
                BannerBoardPlugin.getInstance().getServer().dispatchCommand(BannerBoardPlugin.getInstance().getServer().getConsoleSender(), command);
            ...
            if (this.hasSetting("playercommand")) {
                ...
                // apply placeholders
                command = BannerBoardPlugin.getInstance().applyPlaceholders(command, clicker);
                BannerBoardPlugin.getInstance().getServer().dispatchCommand(clicker, command);
            ...
            if (this.hasSetting("sendserver")) {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        out.writeUTF(server);
                        clicker.sendPluginMessage(BannerBoardPlugin.getInstance(), "BungeeCord", out.toByteArray());
                    }
